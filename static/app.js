...
function monthValue(monthId,yearId){return `${$(yearId).value}-${String(Number($(monthId).value)+1).padStart(2,'0')}`}
function setMonthControls(monthId,yearId,value=selectedMonth){const [y,m]=value.split('-').map(Number);$(monthId).innerHTML=MONTHS.map((n,i)=>`<option value="${i}" ${i===m-1?'selected':''}>${n}</option>`).join('');$(yearId).value=y}
function initMonthControls(){[['dashMonthName','dashYear'],['accountMonthName','accountYear'],['txMonthName','txYear'],['budgetMonthName','budgetYear'],['reportMonthName','reportYear'],['planningMonthName','planningYear']].forEach(([m,y])=>setMonthControls(m,y));}
...
const accountTypeLabels={checking:'Girokonto',savings:'Sparkonto',cash:'Bargeld',credit_card:'Kreditkarte',paypal:'PayPal'};function renderAccounts(){$('accounts').innerHTML=accountsCache.length?accountsCache.map(a=>`<div class="account-card"><div class="account-head"><span><b>${esc(a.name)}</b><small>${esc(accountTypeLabels[a.type]||a.type)}</small></span><strong>${fmt(a.balance)}</strong></div><div class="account-month"><span>Monatsanfang <b>${fmt(a.month_start_balance)}</b></span><span>Bis heute <b>${fmt(a.balance)}</b></span><span>Monatsende <b>${fmt(a.month_end_balance)}</b></span></div></div>`).join(''):'<p class="muted">Noch keine Konten.</p>'}
function renderDashboardData(d){
  ...
  const cutoffText=cutoffDate.toLocaleDateString('de-DE');
  ...
  $('pending').closest('.metric').title=`Offene Ausgaben vom ${new Date(d.remaining_start+'T12:00:00').toLocaleDateString('de-DE')} bis ${new Date(d.month_end+'T12:00:00').toLocaleDateString('de-DE')}`;
...
async function loadDashboard(){
  const monthAtStart=selectedMonth,url='/api/dashboard?month='+encodeURIComponent(selectedMonth),hit=viewCacheGet(url,20000);
...
function yearSummaryMarkup(months){
  ...
  const rows=(months||[]).map(x=>{
    const monthName=MONTHS[Number(x.month.slice(5))-1]||x.month;
...
function renderUpcoming(rows,dashboard){
  if($('upcomingHint')){
    ...
    $('upcomingHint').textContent=selectedEnd<realToday?'Dieser Monat liegt in der Vergangenheit.':`Noch kommende Termine in diesem Monat: ${actualFrom.toLocaleDateString('de-DE')} – ${selectedEnd.toLocaleDateString('de-DE')}. Maßgeblich ist das echte heutige Datum.`
  }
  $('recurringDash').innerHTML=rows.length?rows.map(r=>`<div class="upcoming-row"><span><b>${r.source==='recurring'?'<span class="recurring-mark" title="Wiederkehrende Buchung" aria-label="Wiederkehrende Buchung">↻</span> ':''}${esc(r.name)}</b><small>${new Date(r.date+'T12:00:00').toLocaleDateString('de-DE')} · ${esc(r.account_name)}${r.source==='recurring'?' · wiederkehrend':''}</small></span><strong class="amount ${r.amount<0?'neg':'pos'}">${fmt(r.amount)}</strong></div>`).join(''):'<p class="muted">Keine weiteren Zahlungen in diesem Monat.</p>';
}
function renderAnalysis(a,dashboard){
  ...
  $('analysisSavingsRate').textContent=`${Number(a.savings_rate_pct||0).toLocaleString('de-DE',{maximumFractionDigits:1})} %`;
  if($('analysisAnnualSavingsRate'))$('analysisAnnualSavingsRate').textContent=`${Number(a.annual_savings_rate_pct||0).toLocaleString('de-DE',{maximumFractionDigits:1})} %`;
  ...
  $('analysisTopCategories').innerHTML=(a.top_expense_categories||[]).length?(a.top_expense_categories||[]).map((x,i)=>`<div class="row analysis-rank"><span><b>${i+1}. ${esc(x.name)}</b></span><strong>${fmt(x.amount)}</strong></div>`).join(''):'<p class="muted">Bis zum Stichtag keine kategorisierten Ausgaben.</p>';
  const months=a.monthly_overview||[];$('analysisMonths').innerHTML=months.slice().reverse().map(m=>{const [y,mo]=m.month.split('-').map(Number);return `<div class="analysis-month-row"><b>${MONTHS[mo-1]} ${y}</b><span class="amount pos">+${fmt(m.income)}</span><span class="amount neg">−${fmt(m.expense)}</span><span class="saving-cell">↗ ${fmt(m.savings||0)}</span><span class="rate-cell">${Number(m.savings_rate_pct||0).toLocaleString('de-DE',{maximumFractionDigits:1})} %</span><strong>${fmt(m.net)}</strong></div>`}).join('');drawAnalysisChart(months);
}
function drawAnalysisChart(rows){
  ...
  ctx.font=`${9*ratio}px system-ui`;ctx.textAlign='right';ctx.textBaseline='middle';for(let i=0;i<4;i++){const v=max*(3-i)/3,y=top+ph*i/3;ctx.strokeStyle=grid;ctx.lineWidth=ratio;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(w-right,y);ctx.stroke();ctx.fillStyle=text;ctx.fillText(new Intl.NumberFormat('de-DE',{notation:'compact',maximumFractionDigits:1}).format(v),left-6*ratio,y)}
...
function drawChart(points){
  ...
  function paint(){
    ...
    for(let i=0;i<5;i++){const value=max-span*i/4,y=top+plotH*i/4;ctx.strokeStyle=grid;ctx.lineWidth=ratio;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(w-right,y);ctx.stroke();ctx.fillStyle=text;ctx.fillText(new Intl.NumberFormat('de-DE',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(value),left-8*ratio,y)}
...
async function accountCorrectionDialog(account,month){
  ...
  document.querySelectorAll('[data-cdel]').forEach(b=>b.onclick=async()=>{if(confirm('Monatskorrektur für '+b.dataset.cdel+' löschen?')){await api('/api/accounts/'+account.id+'/month-opening/'+encodeURIComponent(b.dataset.cdel),{method:'DELETE'});modal.close();await loadAccountManager();await loadDashboard();toast('Monatskorrektur gelöscht')}})
}
...
async function attachmentDialog(txId){
  ...
  document.querySelectorAll('[data-adel]').forEach(b=>b.onclick=async()=>{if(confirm('Beleg löschen?')){await api('/api/attachments/'+b.dataset.adel,{method:'DELETE'});modal.close();attachmentDialog(txId)}})
}
...
function bindTransactionRows(rows){
  document.querySelectorAll('[data-edit]').forEach(b=>{const row=rows.find(x=>x.id===Number(b.dataset.edit));b.onclick=()=>row?.transfer?transferDialog(row.transfer_id):txDialog(row)});
  document.querySelectorAll('[data-cancel]').forEach(b=>{const row=rows.find(x=>x.id===Number(b.dataset.cancel));b.onclick=async()=>{if(row?.transfer){if(confirm('Gesamten Transfer auf beiden Konten stornieren?'))await api('/api/transfers/'+row.transfer_id,{method:'DELETE'})}else if(confirm('Buchung stornieren? Sie wird aus der normalen Liste ausgeblendet, bleibt aber in der Historie erhalten.'))await api('/api/transactions/'+b.dataset.cancel,{method:'DELETE'});await loadTransactions();await loadDashboard()}});
  document.querySelectorAll('[data-attach]').forEach(b=>b.onclick=()=>attachmentDialog(Number(b.dataset.attach)));
  document.querySelectorAll('[data-delete]').forEach(b=>{const row=rows.find(x=>x.id===Number(b.dataset.delete));b.onclick=async()=>{if(row?.transfer){if(confirm('Transfer auf beiden Konten endgültig löschen?'))await api('/api/transfers/'+row.transfer_id+'?hard=true',{method:'DELETE'})}else if(confirm('Buchung endgültig löschen? Eine verknüpfte Wiederholungsserie bleibt bestehen und kann darunter separat gelöscht werden.'))await api('/api/transactions/'+b.dataset.delete+'?hard=true',{method:'DELETE'});await loadTransactions();await loadRecurring();await loadDashboard()}});
}
async function loadTransactions(resetPage=false){
  ...
  $('txBody').innerHTML=rows.map(t=>`<tr><td>${esc(t.booking_date)}</td><td><b>${esc(t.name||t.recurring_name||'Buchung')}</b>${t.tags?.length?`<small class="table-sub">${t.tags.map(esc).join(' · ')}</small>`:''}</td><td>${esc(t.account_name)}</td><td>${esc(t.transfer?(t.transfer_side==='out'?t.transfer_to_account_name:t.transfer_from_account_name):(t.payee||'—'))}</td><td>${t.transfer?'<span class="muted">Interner Transfer</span>':esc(t.category_name||'—')}</td><td>${t.transfer?`<span class="badge transfer-badge">↔ Transfer ${t.transfer_side==='out'?'Ausgang':'Eingang'}</span>`:(t.recurring?`<span class="badge"><span class="recurring-mark">↻</span>${esc(({monthly:'Monatlich',weekly:'Wöchentlich',yearly:'Jährlich',daily:'Täglich'})[t.recurring_frequency]||t.recurring_frequency)}</span>`:'—')}</td><td class="right amount ${t.amount<0?'neg':'pos'}">${fmt(t.amount)}</td><td class="actions"><button data-edit="${t.id}">Bearbeiten</button><button class="ghost" data-cancel="${t.id}">Storno</button><button class="ghost" data-attach="${t.id}">Belege</button><button class="ghost" data-delete="${t.id}">Löschen</button></td></tr>`).join('')||'<tr><td colspan="8">Keine Buchungen.</td></tr>';
...
function recurringOverrideDialog(rec){
  ...
  openModal(`<h2>Monatsbetrag anpassen</h2><p class="muted">${esc(rec.name)} · nur dieser Termin wird geändert. Die wiederkehrende Planung selbst bleibt unverändert.</p><label>Termin<input name="due_date" type="date" required value="${due}"></label><label>Betrag EUR<input name="amount" type="number" min="0" step="0.01" required value="${Math.abs(Number(rec.amount)).toFixed(2)}"></label><label>Notiz<input name="note" placeholder="z. B. tatsächliches Gehalt in diesem Monat"></label><small>Im nächsten Monat gilt automatisch wieder der normale Serienbetrag, sofern du dort keinen eigenen Monatsbetrag hinterlegst.</small>`,async f=>{await api('/api/recurring/'+rec.id+'/override',{method:'PUT',body:JSON.stringify({due_date:f.get('due_date'),amount:String(f.get('amount')),note:f.get('note')||null})});await loadRecurring();await loadDashboard();toast('Monatsbetrag gespeichert')});
}
async function loadRecurring(){try{const rows=await api('/api/recurring');const amap=Object.fromEntries(accountsCache.map(a=>[a.id,a.name])),freqLabel={monthly:'Monatlich',weekly:'Wöchentlich',yearly:'Jährlich',daily:'Täglich'};if($('txRecurringCount'))$('txRecurringCount').textContent=String(rows.length);$('recBody').innerHTML=rows.map(r=>`<tr><td>${esc(r.next_date)}${r.status==='overdue'?'<small class="recurring-overdue">überfällig</small>':''}</td><td class="recurring-name-cell"><b>${esc(r.name)}</b><small class="recurring-series-meta">Serie #${r.series_id} · seit ${esc(r.first_date)}${r.executed_count?` · ${r.executed_count}× gebucht`:''}</small>${r.future_change_from?`<small class="recurring-future-change">Änderung vorgemerkt ab ${esc(r.future_change_from)}</small>`:''}</td><td>${esc(amap[r.account_id]||'?')}</td><td>${esc(freqLabel[r.frequency]||r.frequency)}</td><td>${r.valid_until?esc(r.valid_until):'Unbegrenzt'}</td><td class="right amount ${r.amount<0?'neg':'pos'}">${fmt(r.amount)}</td><td class="actions"><button data-redit="${r.id}">Serie bearbeiten</button><button class="ghost" data-roverride="${r.id}">Monat anpassen</button><button data-exec="${r.id}">Als gebucht markieren</button><button class="ghost" data-stop="${r.id}">Stoppen</button><button class="ghost danger-outline" data-rdel="${r.id}">Serie löschen</button></td></tr>`).join('')||'<tr><td colspan="7">Keine wiederkehrenden Buchungen.</td></tr>';document.querySelectorAll('[data-redit]').forEach(b=>b.onclick=()=>recDialog(rows.find(r=>r.id===Number(b.dataset.redit))));document.querySelectorAll('[data-roverride]').forEach(b=>b.onclick=()=>recurringOverrideDialog(rows.find(r=>r.id===Number(b.dataset.roverride))));document.querySelectorAll('[data-exec]').forEach(b=>b.onclick=async()=>{await api('/api/recurring/'+b.dataset.exec+'/execute',{method:'POST'});await loadRecurring();await loadTransactions();await loadDashboard()});document.querySelectorAll('[data-stop]').forEach(b=>b.onclick=async()=>{if(confirm('Serie ab jetzt stoppen? Bereits gebuchte Buchungen bleiben erhalten.')){await api('/api/recurring/'+b.dataset.stop,{method:'DELETE'});await loadRecurring();await loadDashboard()}});document.querySelectorAll('[data-rdel]').forEach(b=>b.onclick=async()=>{if(confirm('Wiederholungsserie vollständig löschen? Bereits erzeugte Buchungen bleiben erhalten.')){await api('/api/recurring/'+b.dataset.rdel+'?hard=true',{method:'DELETE'});await loadRecurring();await loadTransactions();await loadDashboard();toast('Serie gelöscht – bestehende Buchungen bleiben erhalten.')}})}catch(err){if($('txRecurringCount'))$('txRecurringCount').textContent='!';$('recBody').innerHTML=`<tr><td colspan="7"><span class="neg">Serien konnten nicht geladen werden:</span> ${esc(err.message)}</td></tr>`}}



const budgetStrategyInfo={
  hybrid:{title:'Hybrid',description:'Freier Modus: Du kombinierst Kategorie-Limits, ein Gesamtbudget und Sparziele so, wie es zu deinem Haushalt passt. Gut, wenn dein Einkommen oder deine Ausgaben schwanken.'},
  zero_based:{title:'Zero-Based Budgeting',description:'Jeder erwartete Euro bekommt vor Monatsbeginn eine Aufgabe. Du verteilst die erwarteten Einnahmen auf Kategorien und Sparen, bis „nicht verplant“ möglichst 0 € beträgt.'},
  envelope:{title:'Envelope / Umschlag',description:'Für jede Ausgabenkategorie legst du einen festen Topf fest. HaushaltPro zeigt pro Topf, wie viel bereits verbraucht und wie viel noch verfügbar ist.'},
  '50_30_20':{title:'50/30/20-Regel',description:'Richtwert für das Nettoeinkommen: 50 % Grundbedarf, 30 % Wünsche, 20 % Sparen/Rücklagen. Ordne Budgets den Bereichen Bedarf, Wünsche oder Sparen zu.'},
  pay_yourself_first:{title:'Pay Yourself First',description:'Sparen kommt zuerst. Lege zuerst ein Budget im Bereich „Sparen/Rücklagen“ fest; nur der Rest der erwarteten Einnahmen steht für Ausgaben zur Verfügung.'}
};
const budgetBucketLabels={free:'Frei',needs:'Grundbedarf',wants:'Wünsche',savings:'Sparen/Rücklagen'};
...
function renderBudgetStrategy(data){
  const info=budgetStrategyInfo[data.strategy]||budgetStrategyInfo.hybrid;$('budgetStrategy').value=data.strategy;$('budgetStrategyTitle').textContent=info.title;$('budgetStrategyDescription').textContent=info.description;$('budgetIncome').textContent=fmt(data.expected_income);$('budgetAllocated').textContent=fmt(data.category_budget_total);$('budgetSpent').textContent=fmt(data.booked_expense);
  const m=data.strategy_metrics||{};let html='';
  if(data.strategy==='zero_based')html=`<span>Verplant <b>${fmt(m.allocated)}</b></span><span>Nicht verplant <b class="${Number(m.unallocated)<0?'amount neg':''}">${fmt(m.unallocated)}</b></span>`;
  else if(data.strategy==='envelope')html=`<span>In Umschlägen <b>${fmt(m.allocated)}</b></span><span>Noch verfügbar <b>${fmt(m.remaining)}</b></span>`;
  else if(data.strategy==='50_30_20')html=`<span>Bedarf: Ziel ${fmt(m.targets?.needs)} · geplant ${fmt(m.allocated?.needs)}</span><span>Wünsche: Ziel ${fmt(m.targets?.wants)} · geplant ${fmt(m.allocated?.wants)}</span><span>Sparen: Ziel ${fmt(m.targets?.savings)} · geplant ${fmt(m.allocated?.savings)}</span>`;
  else if(data.strategy==='pay_yourself_first')html=`<span>Sparen zuerst <b>${fmt(m.savings_planned)}</b></span><span>Danach verfügbar <b>${fmt(m.after_savings)}</b></span>`;
  else html=`<span>Geplante Kategorien <b>${fmt(m.allocated)}</b></span><span>Erwartete Einnahmen <b>${fmt(m.income)}</b></span>`;
  $('budgetStrategyMetrics').innerHTML=html;
}
async function loadBudgets(){const budgetMonth=monthValue('budgetMonthName','budgetYear');const data=await api('/api/budgets?month='+encodeURIComponent(budgetMonth));renderBudgetStrategy(data);const rows=data.entries||[];$('budgetList').innerHTML=rows.length?rows.map(b=>`<div class="budget-row"><div><b>${esc(b.category_name||'Gesamtbudget')}</b><small>${esc(budgetBucketLabels[b.bucket]||'Frei')}${b.note?' · '+esc(b.note):''}</small></div><div class="budget-progress"><div><span>Budget ${fmt(b.amount)}</span><span>Gebucht ${fmt(b.spent)}</span><span>Rest <b class="${Number(b.remaining)<0?'amount neg':'amount pos'}">${fmt(b.remaining)}</b></span></div><progress max="100" value="${Math.min(100,Math.max(0,Number(b.usage_pct||0)))}"></progress></div><div class="actions"><button data-bedit="${b.id}">Bearbeiten</button><button class="ghost danger-outline" data-bdel="${b.id}">Löschen</button></div></div>`).join(''):'<p class="muted">Noch keine Budgets für diesen Monat. Lege für wichtige Ausgabenkategorien ein Limit fest.</p>';document.querySelectorAll('[data-bedit]').forEach(b=>b.onclick=()=>budgetDialog(rows.find(x=>x.id===Number(b.dataset.bedit)),data.strategy));document.querySelectorAll('[data-bdel]').forEach(b=>b.onclick=async()=>{if(confirm('Budget löschen?')){await api('/api/budgets/'+b.dataset.bdel,{method:'DELETE'});await loadBudgets()}})}
...
function renderCategories(items=[]){if(!$('categoryList'))return;const sums=new Map(items.map(x=>[String(x.category_id),Number(x.amount)]));$('categoryList').innerHTML=categoriesCache.map(c=>`<div class="row category-row"><span><span class="category-title"><b>${esc(c.name)}</b><span class="category-type">${categoryTypeLabel(c.direction)}</span></span></span><span class="category-sum ${c.direction==='expense'?'amount neg':'amount pos'}">${fmt(sums.get(String(c.id))||0)}</span></div>`).join('')||'<p class="muted">Keine Kategorien.</p>'}
...
async function loadReports(){
  ...
  const yearly=period==='year';$('reportSavingsRateCard').hidden=!yearly;$('reportAvgSavingsCard').hidden=!yearly;if(yearly){$('reportSavingsRate').textContent=`${Number(r.savings_rate_pct||0).toLocaleString('de-DE',{maximumFractionDigits:1})} %`;$('reportAvgSavings').textContent=fmt(r.average_saved)}
  const expenseItems=r.items.filter(x=>x.direction==='expense'),incomeItems=r.items.filter(x=>x.direction==='income'),row=x=>`<div class="row"><span><b>${esc(x.category_name)}</b></span><strong>${fmt(x.amount)}</strong></div>`;
  $('reportExpenses').classList.add('report-list');$('reportIncomes').classList.add('report-list');$('reportExpenses').innerHTML=expenseItems.map(row).join('')||'<p class="muted">Keine Ausgaben.</p>';$('reportIncomes').innerHTML=incomeItems.map(row).join('')||'<p class="muted">Keine Einnahmen.</p>';$('reportSavingsList').innerHTML=r.items.filter(x=>x.direction==='savings').map(row).join('')||'<p class="muted">Kein Sparen.</p>';
...
async function loadInvestments(){try{const r=await api('/api/investments');$('invCost').textContent=fmt(r.total_cost);$('invValue').textContent=fmt(r.total_value);$('invPerf').textContent=`${fmt(r.gain)} · ${Number(r.performance_pct).toFixed(2)} %`;$('investmentBody').innerHTML=r.assets.map(a=>`<tr><td><b>${esc(a.name)}</b><small>${esc(a.symbol||'')}</small></td><td>${esc(investmentTypeLabels[a.asset_type]||a.asset_type)}</td><td class="right">${Number(a.quantity).toLocaleString('de-DE',{maximumFractionDigits:8})}</td><td class="right">${fmt(a.purchase_price)}</td><td class="right">${fmt(a.current_price)}</td><td class="right">${fmt(a.market_value)}</td><td class="right amount ${a.gain<0?'neg':'pos'}">${fmt(a.gain)} · ${Number(a.performance_pct).toFixed(2)} %</td><td class="actions"><button data-iedit="${a.id}">Bearbeiten</button><button class="ghost" data-idel="${a.id}">Löschen</button></td></tr>`).join('')||'<tr><td colspan="8">Noch keine Investments.</td></tr>';document.querySelectorAll('[data-iedit]').forEach(b=>b.onclick=()=>investmentDialog(r.assets.find(a=>a.id===Number(b.dataset.iedit))));document.querySelectorAll('[data-idel]').forEach(b=>b.onclick=async()=>{if(confirm('Investment löschen?')){await api('/api/investments/'+b.dataset.idel,{method:'DELETE'});loadInvestments()}})}catch(err){if(err.message.includes('deaktiviert'))switchView('settings');else toast(err.message)}}
...
async function loadPlanningExtras(){
  ...
  $('planningVarianceTable').innerHTML=variance.rows.length?variance.rows.slice(0,12).map(x=>`<div class="row"><span><b>${esc(x.name)}</b><small>${esc(categoryTypeLabel(x.type))}</small></span><span>Plan ${fmt(x.planned)} · Ist ${fmt(x.actual)} · <b>${fmt(x.difference)}</b>${x.difference_pct===null?'':` · ${Number(x.difference_pct).toFixed(1)} %`}</span></div>`).join(''):'<p class="muted">Noch keine Plan/Ist-Daten.</p>';
  $('runwayLiquid').textContent=fmt(runway.liquid_balance);$('runwayFixed').textContent=fmt(runway.monthly_fixed_costs);$('runwayMonths').textContent=runway.runway_months==null?'–':`${Number(runway.runway_months).toLocaleString('de-DE',{maximumFractionDigits:1})} Monate`;
  ...
  $('contractsList').innerHTML=contracts.length?contracts.map(c=>{const deadline=c.cancellation_deadline?new Date(c.cancellation_deadline+'T12:00:00'):null,days=deadline?Math.ceil((deadline-new Date())/86400000):null,urgent=days!==null&&days>=0&&days<=60;return `<div class="row ${urgent?'check-warning':''}"><span><b>${esc(c.title)}</b><small>${esc(c.provider||'')}${deadline?' · kündigen bis '+deadline.toLocaleDateString('de-DE'):''}${urgent?' · '+days+' Tage':''}</small></span><span><button data-cedit="${c.id}">Bearbeiten</button><button class="ghost" data-cdel="${c.id}">Archivieren</button></span></div>`}).join(''):'<p class="muted">Noch keine Vertragsfristen hinterlegt.</p>';
  document.querySelectorAll('[data-cedit]').forEach(b=>b.onclick=()=>contractDialog(contracts.find(x=>x.id===Number(b.dataset.cedit))));document.querySelectorAll('[data-cdel]').forEach(b=>b.onclick=async()=>{if(confirm('Vertrag archivieren?')){await api('/api/contracts/'+b.dataset.cdel,{method:'DELETE'});loadPlanningExtras()}})
}
...
async function loadPlanning(){
  ...
  $('planningFixedRatio').textContent=Number(d.fixed_cost_ratio_pct||0).toLocaleString('de-DE',{maximumFractionDigits:1})+' %';$('planningFixedAmount').textContent='Fixkosten '+fmt(d.fixed_costs);
  ...
  $('planningWarnings').innerHTML=d.liquidity_warnings.length?d.liquidity_warnings.map(x=>`<div class="row"><span><b>${esc(x.account)}</b><small>${new Date(x.date+'T12:00:00').toLocaleDateString('de-DE')}</small></span><strong class="amount neg">${fmt(x.balance)}</strong></div>`).join(''):'<p class="muted">In den nächsten zwölf Monaten wird nach aktueller Planung kein Konto negativ.</p>';
  const h=$('forecastHorizon').value||'12',rows=d.forecast[h]||[];simpleLineChart('planningChart',rows);$('planningForecastList').innerHTML=rows.map(x=>`<div class="analysis-month-row"><span>${esc(x.month)}</span><strong>${fmt(x.end_balance)}</strong></div>`).join('');
...
function auditFieldLabel(key){return AUDIT_FIELD_LABELS[key]||key.replaceAll('_',' ')}
function auditFieldValue(key,value){
  if(value===null||value===undefined||value==='')return '—';
  if(AUDIT_MONEY_FIELDS.has(key))return fmt(Number(value)/100);
  if(key==='fixed_cost'||key==='active')return Number(value)?'Ja':'Nein';
  if(key==='frequency')return ({monthly:'Monatlich',weekly:'Wöchentlich',yearly:'Jährlich',daily:'Täglich'})[value]||String(value);
  if(key==='confidence')return ({fixed:'Fest',likely:'Wahrscheinlich',estimated:'Geschätzt'})[value]||String(value);
...
function auditChangeRows(x){
  ...
  if(Array.isArray(d.deleted_items)&&d.deleted_items.length){
    parts.push(`<details class="audit-deleted-items"><summary>${d.deleted_items.length.toLocaleString('de-DE')} gelöschte Buchungen anzeigen</summary><div class="audit-deleted-list">${d.deleted_items.map(v=>`<div><b>#${v.id} · ${esc(v.name||'Buchung')}</b><span>${esc(v.booking_date||'–')} · ${esc(v.account_name||'–')} · ${esc(v.payee||'–')} · ${esc(v.category_name||'–')} · ${fmt(Number(v.amount||0)/100)}</span></div>`).join('')}</div></details>`)
  }
  return parts.join('');
}
function auditDetailsText(x){
  const d=x.details||{},cur=d.current||d.added||d.snapshot||{};
  if(x.entity_type==='transaction'){const bits=[d.name||cur.name||'Buchung',cur.booking_date,cur.account_name,cur.payee,cur.category_name];if(cur.amount!==undefined)bits.push(auditFieldValue('amount',cur.amount));return `Buchung #${x.entity_id||'–'} · `+bits.filter(Boolean).join(' · ')};
  if(x.entity_type==='transfer')return `Transfer #${x.entity_id||'–'} · ${d.name||cur.name||'Transfer'}${cur.booking_date?' · '+cur.booking_date:''}${cur.from_account_name&&cur.to_account_name?' · '+cur.from_account_name+' → '+cur.to_account_name:''}`;
  ...
  if(d.name)return d.name;if(d.title)return d.title;if(d.filename)return d.filename;if(d.month)return `Monat ${d.month}`;
...
async function loadAuditTimeline(reset=false){
  ...
  const rows=d.items||[];
  $('auditTimeline').innerHTML=rows.map(x=>`<div class="audit-entry"><div class="audit-dot"></div><div class="audit-content"><div class="audit-head"><strong>${esc(AUDIT_ACTION_LABELS[x.action]||x.action)}</strong><time>${esc(formatAuditTimestamp(x.created_at))}</time></div><div class="audit-meta"><span class="audit-user">👤 <b>${esc(x.username||'System')}</b></span><span>${esc(auditDetailsText(x))}</span></div>${auditChangeRows(x)}<div class="audit-foot"><small>Audit #${x.id} · Hash ${esc(String(x.entry_hash||'').slice(0,12))}…</small>${d.can_delete?`<button class="ghost danger-outline audit-delete-btn" data-audit-delete="${x.id}" type="button">Logeintrag löschen</button>`:''}</div></div></div>`).join('')||'<p class="muted">Keine passenden Änderungen gefunden.</p>';
...
async function loadUsers(){if(!$('userAccessList'))return;try{
  ...
  document.querySelectorAll('[data-reset-user]').forEach(b=>b.onclick=()=>resetUserPassword(b.dataset.resetUser));document.querySelectorAll('[data-delete-user]').forEach(b=>b.onclick=async()=>{const username=b.dataset.deleteUser;if(!confirm(`Benutzer ${username} dauerhaft löschen? Zugriffe auf alle Haushaltsbücher werden entfernt.`))return;try{await api('/api/users/'+encodeURIComponent(username),{method:'DELETE'});toast('Benutzer gelöscht');await loadUsers();await loadAuditTimeline(true)}catch(err){toast(err.message)}})
...
async function loadPayeePresets(){
  ...
  document.querySelectorAll('[data-payee-delete]').forEach(b=>b.onclick=async()=>{if(confirm('Empfänger aus der Vorauswahl löschen? Bestehende Buchungen bleiben unverändert.')){await api('/api/payees/'+b.dataset.payeeDelete,{method:'DELETE'});await loadPayeePresets()}})
}
...
async function loadStorage(){try{const d=await api('/api/admin/storage');$('dbSize').textContent=bytesFmt(d.database_bytes);$('dbTxCount').textContent=Number(d.transactions).toLocaleString('de-DE');$('dbAttachmentSize').textContent=`${bytesFmt(d.attachments_bytes)} / ${bytesFmt(d.attachment_limit_bytes)}`}catch(err){toast(err.message)}}
