...
const hpDisplayText=text=>window.HaushaltProPrivacy?.displayText(text)??String(text??'');
const hpDisplayNumber=text=>window.HaushaltProPrivacy?.displayNumber(text)??String(text??'');
const canonicalDeleteConfirmation=value=>String(value??'').trim().toUpperCase()==='DELETE'?'LÖSCHEN':String(value??'').trim();
const hpConfirm=message=>window.confirm(hpDisplayText(hpText(message)));
const hpPrompt=(message,defaultValue)=>window.prompt(hpDisplayText(hpText(message)),defaultValue);
const hpAlert=message=>window.alert(hpDisplayText(hpText(message)));
...
window.HaushaltProSyncContext=()=>({csrf,bookId:currentMe?.book?.id||null,username:currentMe?.username||null});
...
async function api(url,opt={}){
  ...
  if(window.HaushaltProOffline){
    const outcome=await window.HaushaltProOffline.request(url,{credentials:'same-origin',...opt,headers},window.HaushaltProSyncContext());
...
function showApp(me){currentMe=me;$('auth').hidden=true;$('app').hidden=false;$('userLabel').textContent=`${me.username} · ${me.role==='owner'?'Eigentümer':me.role==='editor'?'Bearbeiter':'Nur Lesen'}`;populateBookSwitcher(me);initMonthControls();loadCommon().catch(err=>toast(err.message));window.HaushaltProOffline?.sync?.(false)}
...
function drawAnalysisChart(rows){
  ...
  ctx.font=`${11*ratio}px system-ui`;ctx.textAlign='right';ctx.textBaseline='middle';for(let i=0;i<4;i++){const v=max*(3-i)/3,y=top+ph*i/3;ctx.strokeStyle=grid;ctx.lineWidth=ratio;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(w-right,y);ctx.stroke();ctx.fillStyle=text;ctx.fillText(hpDisplayNumber(new Intl.NumberFormat(hpLocale(),{notation:'compact',maximumFractionDigits:1}).format(v)),left-6*ratio,y,left-10*ratio)}
...
function openModal(html,onSave,{refresh=true,success='Gespeichert',saveLabel='Speichern'}={}){
  ...
  $('modalContent').innerHTML=html;$('modalForm').reset();window.HaushaltProPrivacy?.scan($('modalContent'));modal.showModal();$('modalCancel').onclick=()=>modal.close();
  let saving=false;
  $('modalForm').onsubmit=async e=>{
    e.preventDefault();if(saving)return;saving=true;$('modalSave').disabled=true;offlineSaveActive=false;
    const offlineSaved=()=>{modal.close();offlineSaveActive=false;toast(hpLang()==='en'?'Saved offline – will sync automatically.':'Offline gespeichert – wird automatisch synchronisiert.');};
    try{
      ...
      if(offlineSaveActive){offlineSaved();return}
      toast(err.message);
    }finally{saving=false;$('modalSave').disabled=false}
  };
}
...
function txDialog(tx=null,forceRecurring=false){
  ...
  const recurringBlock=`<label class="check recurring-check"><input id="txRecurring" name="recurring" type="checkbox" ${isRecurring?'checked':''}> Wiederkehrende Zahlung / Einnahme</label>
    <div id="txRecurringFields" class="subform" ${isRecurring?'':'hidden'}>
      ${recurrenceRuleFields('txRecurrence',freq,intervalCount,'recurring_frequency','recurring_interval_count')}
  ...
  openModal(`<h2>${tx?'Buchung bearbeiten':'Buchung anlegen'}</h2><label>Konto<select name="account_id">${accountOptions(tx?.account_id)}</select></label><label>Name der Buchung<input name="name" maxlength="160" required placeholder="z. B. KFZ-Versicherung" value="${esc(tx?.name||tx?.recurring_name||tx?.payee||'')}"><small>Dieser Name dient zur eindeutigen Dokumentation und Suche.</small></label><label>Betrag EUR<input name="amount" type="number" min="0" step="0.01" required value="${tx?Math.abs(tx.amount):''}"></label><label>Datum<input name="booking_date" type="date" required value="${tx?.booking_date||new Date().toISOString().slice(0,10)}"><small>Liegt das Datum in der Zukunft, erscheint die Buchung ab diesem Tag in der Vorschau.</small></label><label>Empfänger<input name="payee" list="payeeSuggestions" autocomplete="off" placeholder="z. B. REWE, ALDI, Allianz" value="${esc(tx?.payee||'')}"><datalist id="payeeSuggestions">${payeePresetsCache.map(p=>`<option value="${esc(p.name)}"></option>`).join('')}</datalist></label><label class="check remember-payee"><input name="remember_payee" type="checkbox"> Empfänger für spätere Vorauswahl merken</label><small class="muted optin-note">Nur mit diesem Häkchen wird der Empfänger in der bearbeitbaren Vorschlagsliste gespeichert.</small><label>Kategorie<select name="category_id" required>${categoryOptions(tx?.category_id)}</select><small>Die Kategorie bestimmt automatisch, ob die Buchung Einnahme, Ausgabe oder Sparen ist.</small></label>${recurringBlock}<label>Prognose-Sicherheit<select name="confidence"><option value="fixed" ${!tx||tx?.confidence==='fixed'?'selected':''}>Fest</option><option value="likely" ${tx?.confidence==='likely'?'selected':''}>Wahrscheinlich</option><option value="estimated" ${tx?.confidence==='estimated'?'selected':''}>Geschätzt</option></select><small>Relevant für zukünftige Prognosen.</small></label><label class="check"><input name="fixed_cost" type="checkbox" ${tx?.fixed_cost?'checked':''}> Fixkosten / vertraglich gebunden</label><label>Tags (kommagetrennt)<input name="tags" value="${esc((tx?.tags||[]).join(', '))}"></label><label>Notiz<input name="note" value="${esc(tx?.note||'')}"></label>`,async f=>{
    ...
    const body={account_id:Number(f.get('account_id')),amount:String(f.get('amount')),direction:null,booking_date:f.get('booking_date'),name:f.get('name')||null,payee:f.get('payee')||null,note:f.get('note')||null,category_id:f.get('category_id')?Number(f.get('category_id')):null,status:'executed',tags:String(f.get('tags')||'').split(',').map(x=>x.trim()).filter(Boolean),splits:[],recurring,recurring_frequency:recurring?f.get('recurring_frequency'):null,recurring_interval_count:recurring?Number(f.get('recurring_interval_count')||1):1,recurring_until:recurring&&f.get('recurring_until')?f.get('recurring_until'):null,recurring_effective_from:recurring&&f.get('recurring_effective_from')?f.get('recurring_effective_from'):null,confidence:f.get('confidence')||'fixed',fixed_cost:f.get('fixed_cost')==='on',remember_payee:f.get('remember_payee')==='on'};
    const saved=await api(tx?'/api/transactions/'+tx.id:'/api/transactions',{method:tx?'PUT':'POST',body:JSON.stringify(body)});if(saved.offline_queued)return;
    await loadTransactions();await loadRecurring();
  });
  bindRecurrencePreset('txRecurrence','recurring_frequency','recurring_interval_count');
...
async function transferDialog(transferId=null){
  ...
  openModal(`<h2>${tr?'Transfer bearbeiten':'Transfer anlegen'}</h2><p class="muted">Interne Umbuchung zwischen deinen eigenen Konten. Sie verändert die Kontostände, zählt aber nicht als Einnahme oder Ausgabe des Haushalts.</p>${seriesNote}<label>Von Konto<select name="from_account_id">${accountOptions(from)}</select></label><label>Auf Konto<select name="to_account_id">${accountOptions(to)}</select></label><label>Betrag EUR<input name="amount" type="number" min="0.01" step="0.01" required value="${tr?Number(tr.amount).toFixed(2):''}"></label><label>Datum<input name="booking_date" type="date" required value="${tr?.booking_date||today}"></label><label>Name<input name="name" maxlength="160" required value="${esc(tr?.name||'Interner Transfer')}"></label><label>Notiz<input name="note" value="${esc(tr?.note||'')}"></label>${repeatFields}`,async f=>{
    ...
    const saved=await api(tr?'/api/transfers/'+tr.id:'/api/transfers',{method:tr?'PUT':'POST',body:JSON.stringify(body)});if(saved.offline_queued)return;await loadTransactions();await loadRecurring();await loadDashboard();
  });
  if(!tr){
...
function recurringTransactionOverrideDialog(tx){
  openModal(`<h2>${esc(hpText('Einzelnen Serientermin anpassen'))}</h2><p class="muted">${esc(hpText('Diese Änderung gilt nur für diesen Termin. Die Serienvorlage bleibt unverändert.'))}</p><small>${esc(hpText('Ursprünglicher Serientermin'))}: ${esc(formatDateValue(tx.recurring_due_date||tx.booking_date))}</small><label>${esc(hpText('Buchungsdatum'))}<input name="booking_date" type="date" required value="${esc(tx.booking_date)}"><small>${esc(hpText('Nur diese Buchung verschieben – auch in einen anderen Monat.'))}</small></label><label>${esc(hpText('Betrag EUR'))}<input name="amount" type="number" min="0" step="0.01" required value="${Math.abs(Number(tx.amount)).toFixed(2)}"></label><label>${esc(hpText('Notiz'))}<input name="note" value="${esc(tx.note||'')}"></label><small>${esc(hpText('Die Serienänderung ab einem Stichtag erfolgt unter „Wiederkehrende Buchungen“.'))}</small>`,async f=>{await api('/api/recurring/'+tx.recurring_id+'/override',{method:'PUT',body:JSON.stringify({due_date:tx.recurring_due_date||tx.booking_date,booking_date:f.get('booking_date'),amount:String(f.get('amount')),note:f.get('note')||null})});await loadTransactions();await loadRecurring();await loadDashboard();toast(hpText('Einzeltermin gespeichert'))});
}
...
function drawDonut(canvasId,legendId,items){
  ...
  canvas.onpointermove=e=>{if(e.pointerType==='touch')return;setActive(hit(e))};canvas.onpointerleave=e=>{if(e.pointerType!=='touch')setActive(null)};canvas.onpointerdown=e=>{const i=hit(e);setActive(state.active===i?null:i)};canvas._hpPrivacyPaint=paint;paint();
}
...
function interactiveMonthlyChart(canvasId,rows,key='end_balance',labelKey='month'){
  ...
  const paint=()=>{
    ...
    for(let i=0;i<=4;i++){const y=top+plotH*i/4,v=max-span*i/4;ctx.strokeStyle=grid;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(W-right,y);ctx.stroke();ctx.fillStyle=muted;ctx.textAlign='right';ctx.fillText(hpDisplayText(fmt(v)),left-9,y)}
    ...
    if(state.hover!==null&&rows[state.hover]){
      ...
      ctx.fillStyle=fg;ctx.textAlign='right';ctx.textBaseline='middle';ctx.fillText(hpDisplayText(fmt(v)),left-9,y);
      ctx.textAlign='center';ctx.textBaseline='top';const raw=String(r[labelKey]||'');ctx.fillText(raw,x,H-bottom+31);
      const label=hpDisplayText(`${raw} · ${fmt(v)}`);ctx.font='12px system-ui';const tw=ctx.measureText(label).width+20,th=30,tx=Math.min(Math.max(left,x-tw/2),W-right-tw),ty=Math.max(top,y-42);
  ...
  c.onmousemove=e=>hoverAt(e.clientX);c.onmouseleave=()=>{state.hover=null;paint()};c.ontouchmove=e=>{if(e.touches[0])hoverAt(e.touches[0].clientX)};c._hpPrivacyPaint=paint;paint();
}
