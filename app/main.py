...
def transfer_get(transfer_id: int, request: Request):
    session(request); snap=transfer_snapshot(db.db(),transfer_id); snap["amount"]=euros(snap["amount"]); return snap


@app.post("/api/transfers")
def transfer_create(x: TransferIn, request: Request):
    sess=session(request,True); amount=abs(cents(x.amount)); ts=iso(utcnow())
    if amount<=0: raise HTTPException(400,"Transferbetrag muss größer als 0 sein")
    with db.transaction() as c:
        ...
        cur=c.execute("""INSERT INTO transfers(from_account_id,to_account_id,amount,booking_date,name,note,active,created_at,updated_at)
                         VALUES(?,?,?,?,?,?,1,?,?)""",(x.from_account_id,x.to_account_id,amount,x.booking_date.isoformat(),name,note,ts,ts))
        tid=cur.lastrowid
        c.execute("""INSERT INTO transactions(account_id,amount,direction,booking_date,name,payee,note,category_id,status,external_id,recurring_id,transfer_id,transfer_side,confidence,fixed_cost,created_at,updated_at)
                     VALUES(?,?,?,?,?,?,?,NULL,'executed',?,NULL,?,'out','fixed',0,?,?)""",
                  (x.from_account_id,-amount,'expense',x.booking_date.isoformat(),name,dst["name"],note,f"transfer-{tid}-out",tid,ts,ts))
        c.execute("""INSERT INTO transactions(account_id,amount,direction,booking_date,name,payee,note,category_id,status,external_id,recurring_id,transfer_id,transfer_side,confidence,fixed_cost,created_at,updated_at)
                     VALUES(?,?,?,?,?,?,?,NULL,'executed',?,NULL,?,'in','fixed',0,?,?)""",
                  (x.to_account_id,amount,'income',x.booking_date.isoformat(),name,src["name"],note,f"transfer-{tid}-in",tid,ts,ts))
        snap=transfer_snapshot(c,tid)
        audit_append(c,sess[1],"transfer.create","transfer",tid,{"name":name,"added":audit_pick(snap,("name","amount","booking_date","from_account_name","to_account_name","note"))})
    return {"id":tid}
...
def transfer_update(transfer_id: int, x: TransferIn, request: Request):
    ...
    with db.transaction() as c:
        ...
        if not int(before.get("active",1)): raise HTTPException(400,"Transfer ist storniert")
        src,dst=validate_transfer_accounts(c,x.from_account_id,x.to_account_id)
        name=clean_text(x.name,160) or "Transfer"; note=clean_text(x.note,1000)
        ...
        c.execute("""UPDATE transactions SET account_id=?,amount=?,direction='expense',booking_date=?,name=?,payee=?,note=?,updated_at=?
                     WHERE transfer_id=? AND transfer_side='out'""",(x.from_account_id,-amount,x.booking_date.isoformat(),name,dst["name"],note,ts,transfer_id))
        c.execute("""UPDATE transactions SET account_id=?,amount=?,direction='income',booking_date=?,name=?,payee=?,note=?,updated_at=?
                     WHERE transfer_id=? AND transfer_side='in'""",(x.to_account_id,amount,x.booking_date.isoformat(),name,src["name"],note,ts,transfer_id))
        after=transfer_snapshot(c,transfer_id)
        fields=("name","amount","booking_date","from_account_name","to_account_name","note")
        audit_append(c,sess[1],"transfer.update","transfer",transfer_id,{"name":name,"changes":audit_changes(before,after,fields),"current":audit_pick(after,fields)})
    return {"ok":True}
...
def transfer_delete(transfer_id: int, request: Request, hard: bool=False):
    ...
    with db.transaction() as c:
        before=transfer_snapshot(c,transfer_id)
        audit_append(c,sess[1],"transfer.delete","transfer",transfer_id,{"snapshot":audit_pick(before,("name","amount","booking_date","from_account_name","to_account_name","note"))})
        if hard:
            c.execute("DELETE FROM transactions WHERE transfer_id=?",(transfer_id,)); c.execute("DELETE FROM transfers WHERE id=?",(transfer_id,))
        else:
            c.execute("UPDATE transfers SET active=0,updated_at=? WHERE id=?",(iso(utcnow()),transfer_id)); c.execute("UPDATE transactions SET status='cancelled',updated_at=? WHERE transfer_id=?",(iso(utcnow()),transfer_id))
    return {"ok":True}
...
def occurrences(start: date, frequency: str, from_day: date, to_day: date):
    """Yield occurrences without letting short months permanently move the anchor day.

    A monthly series started on the 31st becomes Feb 28/29, then Mar 31 again.
    """
    if from_day > to_day:
        return
    if frequency == "daily":
        step = max(0, (from_day - start).days)
        ...
        return
    if frequency == "weekly":
        delta = max(0, (from_day - start).days)
        step = max(0, delta // 7)
        d = start + timedelta(days=step * 7)
        while d < from_day:
            d += timedelta(days=7)
        while d <= to_day:
            yield d
        ...
        return
    if frequency == "monthly":
        n = max(0, (from_day.year - start.year) * 12 + from_day.month - start.month - 1)
        while True:
        ...
        return
    if frequency == "yearly":
        n = max(0, from_day.year - start.year - 1)
        while True:
            try:
                d = start.replace(year=start.year + n)
            except ValueError:
                d = start.replace(year=start.year + n, day=28)
            if d > to_day:
                break
            if d >= from_day:
                yield d
...
def recurring_events(c, account_id: int | None, from_day: date, to_day: date):
    events=[]
    for r in recurring_rows_for_window(c, account_id, from_day, to_day):
    ...
    with db.transaction() as c:
        ...
        for version in versions:
            ...
            if vf <= x.due_date <= vu:
                ...
                for occurrence in occurrences(anchor, version["frequency"], x.due_date, x.due_date):
...
def planned_month_category_totals(c, start: date, end: date) -> dict[int | None, dict]:
    ...
    One-time manual/future transactions are part of the plan. Transactions
    linked to a recurring series are represented by the series itself so an
    executed amount can differ from the original plan without erasing the
    Plan/Ist variance.
    """
    out={}
    rows=c.execute("""SELECT t.category_id,COALESCE(cat.name,'Nicht kategorisiert') category_label,
    ...
    for r in rows:
        out[r["category_id"]]={"category_id":r["category_id"],"name":r["category_label"],"type":r["typ"],"amount":int(r["amount"] or 0)}
    for r in recurring_rows_for_window(c,None,start,end):
        vf=date.fromisoformat(r["valid_from"] or r["next_date"]); vu=date.fromisoformat(r["valid_until"]) if r["valid_until"] else end
        low=max(start,vf); high=min(end,vu)
        if low>high: continue
        anchor=date.fromisoformat(r["anchor_date"] or r["valid_from"] or r["next_date"])
        cat=c.execute("SELECT name,direction FROM categories WHERE id=?",(r["category_id"],)).fetchone() if r["category_id"] else None
        typ=cat["direction"] if cat else ("income" if r["kind"]=="income" else "expense"); name=cat["name"] if cat else "Nicht kategorisiert"; cid=r["category_id"]
        series_id=r["series_id"] or r["id"]
        for due in occurrences(anchor,r["frequency"],low,high):
            override=c.execute("SELECT amount FROM recurring_overrides WHERE series_id=? AND due_date=?",(series_id,due.isoformat())).fetchone()
            raw=int(override["amount"]) if override else int(r["amount"])
            val=abs(recurring_signed_amount(c,r,raw))
            row=out.setdefault(cid,{"category_id":cid,"name":name,"type":typ,"amount":0}); row["amount"]+=val
    return out
...
def planned_fixed_costs(c, start: date, end: date) -> int:
    ...
    for r in recurring_rows_for_window(c,None,start,end):
        ...
        for due in occurrences(anchor,r["frequency"],low,high):
            override=c.execute("SELECT amount FROM recurring_overrides WHERE series_id=? AND due_date=?",(series_id,due.isoformat())).fetchone()
            raw=int(override["amount"]) if override else int(r["amount"])
            signed=recurring_signed_amount(c,r,raw)
            if signed<0:
                total+=abs(signed)
    return total
