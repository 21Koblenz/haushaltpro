...
def is_unlocked() -> bool:
    return _conn is not None or _master_key is not None


def unlock(key: str, initialize: bool = False):
...
def db():
    ...
    if c is None:
        c = _register_connection(connect(_master_key))
...
def migrate_schema(c) -> None:
    ...
    c.execute("INSERT INTO app_meta(key,value) VALUES('schema_version','21') ON CONFLICT(key) DO UPDATE SET value='21'")
    c.commit()
...
def rekey(new_key: str) -> None:
    ...
    if _master_key is None:
        raise RuntimeError("DATABASE_LOCKED")
    old_key = _master_key
    # No managed read connection may keep the old key after REKEY.
    _close_managed_connections()
    with _write_lock:
        c = connect(old_key)
        try:
